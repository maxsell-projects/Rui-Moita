...
<div class="bg-brand-primary text-white pt-32 pb-20 text-center relative overflow-hidden">
    ...
    <div class="container mx-auto px-6 relative z-10" data-aos="fade-up">
        <p class="text-brand-premium font-mono text-xs uppercase tracking-[0.4em] mb-6">
            {{ __('Fiscalidade & Estratégia') }}
        </p>
        <h1 class="text-4xl md:text-6xl font-didot leading-tight">
            {{ __('Simulador de Mais-Valias') }}
        </h1>
        <p class="text-gray-300 font-light max-w-2xl mx-auto mt-6 text-lg">
            {{ __('Antecipe o impacto fiscal da venda do seu imóvel. Calcule a estimativa de IRS a pagar com rigor e transparência.') }}
        </p>
    </div>
</div>
            ...
            <div class="lg:col-span-8 space-y-8">
                ...
                <div class="bg-white p-8 rounded-lg shadow-sm border-t-4 border-brand-premium" data-aos="fade-up">
                    <div class="flex items-center gap-4 mb-8">
                        ...
                        <h3 class="text-xl font-didot text-brand-primary">{{ __('Dados da Aquisição (Compra)') }}</h3>
                    </div>
                    
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-2">{{ __('Valor de Compra (€)') }}</label>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-2">{{ __('Ano') }}</label>
                            ...
                            <div>
                                <label class="block text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-2">{{ __('Mês') }}</label>
                    ...
                    <div class="mt-6 pt-6 border-t border-gray-100">
                        <label class="block text-sm font-medium text-gray-700 mb-3">{{ __('A habitação foi construída pelo próprio?') }}</label>
                        <div class="flex gap-6">
                            <label class="inline-flex items-center cursor-pointer group">
                                ...
                                <span class="ml-2 text-sm text-gray-600 group-hover:text-brand-primary transition-colors">{{ __('Sim') }}</span>
                            </label>
                            <label class="inline-flex items-center cursor-pointer group">
                                <input type="radio" value="Não" x-model="form.self_built" class="peer sr-only">
                                <span class="w-4 h-4 border border-gray-300 rounded-full peer-checked:bg-brand-premium peer-checked:border-brand-premium transition-all"></span>
                                <span class="ml-2 text-sm text-gray-600 group-hover:text-brand-primary transition-colors">{{ __('Não') }}</span>
                            </label>
                        </div>
                        <p class="text-[10px] text-gray-400 mt-2 italic">{{ __('*Afeta o coeficiente de desvalorização da moeda.') }}</p>
                    </div>
                </div>
                ...
                <div class="bg-white p-8 rounded-lg shadow-sm border-t-4 border-brand-premium" data-aos="fade-up" data-aos-delay="100">
                    <div class="flex items-center gap-4 mb-8">
                        ...
                        <h3 class="text-xl font-didot text-brand-primary">{{ __('Dados da Realização (Venda)') }}</h3>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-2">{{ __('Valor de Venda (€)') }}</label>
                ...
                <div class="bg-white p-8 rounded-lg shadow-sm border-t-4 border-brand-premium" data-aos="fade-up" data-aos-delay="200">
                    <div class="flex items-center gap-4 mb-8">
                        ...
                        <h3 class="text-xl font-didot text-brand-primary">{{ __('Despesas Dedutíveis') }}</h3>
                    </div>

                    <div class="mb-6">
                        <label class="block text-sm font-medium text-gray-700 mb-3">{{ __('Existem despesas a deduzir (Obras, IMT, Comissões)?') }}</label>
                        <div class="flex gap-6">
                            <label class="inline-flex items-center cursor-pointer group">
                                <input type="radio" value="Sim" x-model="form.has_expenses" class="peer sr-only">
                                <div class="px-4 py-2 border border-gray-200 rounded text-sm peer-checked:bg-brand-primary peer-checked:text-white peer-checked:border-brand-primary transition-all group-hover:border-brand-primary">{{ __('Sim') }}</div>
                            </label>
                            <label class="inline-flex items-center cursor-pointer group">
                                <input type="radio" value="Não" x-model="form.has_expenses" class="peer sr-only">
                                <div class="px-4 py-2 border border-gray-200 rounded text-sm peer-checked:bg-brand-primary peer-checked:text-white peer-checked:border-brand-primary transition-all group-hover:border-brand-primary">{{ __('Não') }}</div>
                            </label>
                        </div>
                    </div>
                    ...
                    <div x-show="form.has_expenses === 'Sim'" x-transition class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-[#F5F7FA] p-6 rounded border border-gray-200">
                        <div>
                            <label class="block text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-2">{{ __('Obras / Melhorias (€)') }}</label>
                            ...
                            <p class="text-[10px] text-gray-400 mt-1">{{ __('*Últimos 12 anos apenas.') }}</p>
                        </div>
                        <div>
                            <label class="block text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-2">{{ __('IMT e Selo da Compra (€)') }}</label>
                        ...
                        <div>
                            <label class="block text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-2">{{ __('Comissão Imobiliária (€)') }}</label>
                        ...
                        <div>
                            <label class="block text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-2">{{ __('Registo / Notariado (€)') }}</label>
                ...
                <div class="bg-white p-8 rounded-lg shadow-sm border-t-4 border-brand-premium" data-aos="fade-up" data-aos-delay="300">
                    <div class="flex items-center gap-4 mb-8">
                        ...
                        <h3 class="text-xl font-didot text-brand-primary">{{ __('Enquadramento Fiscal') }}</h3>
                    </div>

                    {{-- Venda ao Estado --}}
                    <div class="mb-8 border-b border-gray-100 pb-6">
                        <label class="block text-sm font-bold text-gray-700 mb-3">{{ __('Venda a Entidade Pública (Estado/Autarquias)?') }}</label>
                        <div class="flex gap-6">
                            <label class="inline-flex items-center cursor-pointer group">
                                <input type="radio" value="Sim" x-model="form.sold_to_state" @change="resetHPPFields" class="peer sr-only">
                                <span class="w-4 h-4 border border-gray-300 rounded-full peer-checked:bg-brand-premium peer-checked:border-brand-premium transition-all"></span>
                                <span class="ml-2 text-sm text-gray-600">{{ __('Sim') }}</span>
                            </label>
                            <label class="inline-flex items-center cursor-pointer group">
                                <input type="radio" value="Não" x-model="form.sold_to_state" @change="resetHPPFields" class="peer sr-only">
                                <span class="w-4 h-4 border border-gray-300 rounded-full peer-checked:bg-brand-premium peer-checked:border-brand-premium transition-all"></span>
                                <span class="ml-2 text-sm text-gray-600">{{ __('Não') }}</span>
                            </label>
                        </div>
                        <div x-show="form.sold_to_state === 'Sim'" x-transition class="mt-4 p-3 bg-blue-50 border border-blue-100 text-blue-800 text-xs font-bold rounded">
                            {{ __('ℹ️ Isenção total de IRS aplicável.') }}
                        </div>
                    </div>
                    ...
                    <div x-show="form.sold_to_state === 'Não'" x-transition class="space-y-8">
                        ...
                        {{-- HPP --}}
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-3">{{ __('Status do Imóvel na Venda') }}</label>
                            <div class="space-y-3">
                                <label class="flex items-center cursor-pointer p-3 border border-gray-200 rounded hover:bg-gray-50 transition">
                                    ...
                                    <span class="ml-3 text-sm text-gray-700">{{ __('Habitação Própria Permanente (HPP) há +12 meses') }}</span>
                                </label>
                                <label class="flex items-center cursor-pointer p-3 border border-gray-200 rounded hover:bg-gray-50 transition">
                                    ...
                                    <span class="ml-3 text-sm text-gray-700">{{ __('HPP há menos de 12 meses') }}</span>
                                </label>
                                <label class="flex items-center cursor-pointer p-3 border border-gray-200 rounded hover:bg-gray-50 transition">
                                    <input type="radio" value="Não" x-model="form.hpp_status" @change="resetReinvestmentFields" class="accent-brand-premium w-4 h-4">
                                    <span class="ml-3 text-sm text-gray-700">{{ __('Secundária / Investimento / Terreno') }}</span>
                                </label>
                            </div>
                        </div>
                        ...
                        <div class="bg-[#F5F7FA] p-6 rounded border border-gray-200">
                            <h4 class="text-sm font-bold text-brand-primary uppercase tracking-widest mb-4">{{ __('Reinvestimento & Benefícios') }}</h4>
                            
                            {{-- Reinvestimento --}}
                            <div class="mb-6" x-show="form.hpp_status === 'Sim'">
                                <label class="block text-xs font-bold text-gray-500 mb-2">{{ __('Intenção de Reinvestimento (HPP)') }}</label>
                                <div class="flex gap-4 mb-3">
                                    <label class="inline-flex items-center cursor-pointer"><input type="radio" value="Sim" x-model="form.reinvest_intention" class="accent-brand-premium w-4 h-4"><span class="ml-2 text-sm">{{ __('Sim') }}</span></label>
                                    <label class="inline-flex items-center cursor-pointer"><input type="radio" value="Não" x-model="form.reinvest_intention" class="accent-brand-premium w-4 h-4"><span class="ml-2 text-sm">{{ __('Não') }}</span></label>
                                </div>
                                <div x-show="form.reinvest_intention === 'Sim'" x-transition>
                                    <input type="number" step="0.01" x-model="form.reinvestment_amount" class="w-full border-gray-300 rounded text-sm focus:border-brand-cta focus:ring-0" placeholder="{{ __('Valor a reinvestir (€)') }}">
                                </div>
                            </div>

                            {{-- Amortização --}}
                            <div class="mb-6">
                                <label class="block text-xs font-bold text-gray-500 mb-2">{{ __('Amortização de Crédito (Norma Transitória)') }}</label>
                                <div class="flex gap-4 mb-3">
                                    <label class="inline-flex items-center cursor-pointer"><input type="radio" value="Sim" x-model="form.amortize_credit" class="accent-brand-premium w-4 h-4"><span class="ml-2 text-sm">{{ __('Sim') }}</span></label>
                                    <label class="inline-flex items-center cursor-pointer"><input type="radio" value="Não" x-model="form.amortize_credit" class="accent-brand-premium w-4 h-4"><span class="ml-2 text-sm">{{ __('Não') }}</span></label>
                                </div>
                                <div x-show="form.amortize_credit === 'Sim'" x-transition>
                                    <input type="number" step="0.01" x-model="form.amortization_amount" class="w-full border-gray-300 rounded text-sm focus:border-brand-cta focus:ring-0" placeholder="{{ __('Valor a amortizar (€)') }}">
                                </div>
                            </div>

                            {{-- Rendimentos --}}
                            <div>
                                <label class="block text-xs font-bold text-gray-500 mb-2">{{ __('Rendimento Anual Coletável (IRS - Anexo A)') }}</label>
            ...
            <div class="lg:col-span-4" id="results-area">
                <div class="sticky top-32 space-y-6">
                    ...
                    <div x-show="!hasCalculated" class="bg-white border-2 border-dashed border-gray-200 rounded-lg p-10 text-center text-gray-400">
                        ...
                        <p class="text-xs uppercase tracking-widest">{{ __('A aguardar dados...') }}</p>
                    </div>
                    ...
                    <div x-show="hasCalculated" x-transition style="display: none;" class="space-y-6">
                        ...
                        <div class="bg-brand-primary text-white rounded-lg p-8 shadow-2xl relative overflow-hidden">
                            ...
                            <h3 class="text-xs font-bold text-brand-premium uppercase tracking-[0.2em] mb-2">{{ __('Imposto Estimado (IRS)') }}</h3>
                            ...
                            <div class="space-y-3 text-sm font-light border-t border-white/10 pt-4">
                                <div class="flex justify-between">
                                    <span class="text-gray-400">{{ __('Mais-Valia Bruta') }}</span>
                                    <span x-text="results.gross_gain_fmt + ' €'"></span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-gray-400">{{ __('Base Tributável (50%)') }}</span>
                        ...
                        <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden text-sm">
                            ...
                            <div class="p-6 space-y-3">
                                ...
                                <div class="flex justify-between text-brand-secondary font-bold" x-show="results.reinvestment_fmt !== '0,00' || form.amortize_credit === 'Sim'">
                                    <span>{{ __('Isenção (Reinvest/Amort)') }}</span>
                        ...
                        <div class="p-4 bg-brand-premium/10 border border-brand-premium/30 rounded text-xs text-brand-primary leading-relaxed">
                            <strong class="block mb-1 font-bold">{{ __('Nota Técnica:') }}</strong>
                            {{ __('A tributação incide sobre 50% da mais-valia líquida para residentes fiscais em Portugal, sendo englobada aos restantes rendimentos.') }}
                        </div>
            ...
            <div x-show="showLeadModal" x-transition.scale class="relative inline-block bg-white rounded-lg text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:max-w-lg w-full border-t-4 border-brand-premium">
                <div class="bg-white px-8 py-10">
                    <div class="text-center mb-8">
                        <h3 class="text-2xl font-didot text-brand-primary mb-2">{{ __('Relatório Detalhado') }}</h3>
                        <p class="text-sm text-gray-500 font-light">{{ __('Para aceder à análise completa e receber o PDF oficial, por favor identifique-se.') }}</p>
                    </div>
                    
                    <div class="space-y-6">
                        <div>
                            <label class="block text-[10px] uppercase tracking-widest text-gray-400 mb-2">{{ __('Nome Completo') }}</label>
                        ...
                        <div>
                            <label class="block text-[10px] uppercase tracking-widest text-gray-400 mb-2">{{ __('Email Corporativo / Pessoal') }}</label>
...
<script>
    function gainsForm() {
        return {
            i18n: {
                fillValues: "{{ __('Por favor, preencha os valores de Aquisição e Venda.') }}",
                fillContact: "{{ __('Por favor, preencha o seu nome e email.') }}",
                error: "{{ __('Ocorreu um erro no cálculo. Verifique os dados inseridos.') }}"
            },
            hasCalculated: false,
            showLeadModal: false,
            form: {
