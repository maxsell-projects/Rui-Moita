...
<div x-data="{ activeModal: null }" @keydown.escape.window="activeModal = null">
        ...
        <div class="container mx-auto px-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 md:gap-8">
                ...
                @forelse($consultants as $consultant)
                    <div class="group cursor-pointer" @click="activeModal = {{ $consultant->id }}">
                        {{-- Foto Compacta --}}
                        <div class="aspect-square relative overflow-hidden rounded-sm bg-gray-100 mb-4 shadow-sm group-hover:shadow-md transition-all duration-300">
                        ...
                        <div class="text-center">
                            <h3 class="font-serif text-lg text-brand-secondary group-hover:text-brand-accent transition-colors">{{ $consultant->name }}</h3>
                            <p class="text-[10px] uppercase tracking-widest text-gray-400 font-bold mt-1">{{ $consultant->role }}</p>
                        </div>
                    </div>
                @empty
    ...
    {{-- Renderizamos fora da grid para evitar problemas de Z-Index --}}
        ...
        <div x-show="activeModal === {{ $consultant->id }}" 
            ...
            <div class="relative bg-white w-full max-w-4xl rounded-sm shadow-2xl overflow-hidden flex flex-col md:flex-row max-h-[90vh]"
                ...
                <div class="w-full md:w-5/12 h-64 md:h-auto relative bg-gray-100">
                    <img src="{{ $consultant->photo_url }}" class="absolute inset-0 w-full h-full object-cover">
                </div>
                ...
                <div class="w-full md:w-7/12 p-8 md:p-12 overflow-y-auto custom-scrollbar bg-white">
                    ...
                    <div class="mb-8 border-b border-gray-100 pb-6">
                        <span class="text-brand-accent uppercase tracking-[0.2em] text-[10px] font-bold mb-2 block">{{ $consultant->role }}</span>
                    ...
                    <div class="space-y-4">
                        ...
                        @if($consultant->phone)
                            <div class="flex items-center gap-4 group">
                                <div class="w-10 h-10 bg-brand-surface rounded-full flex items-center justify-center text-brand-secondary group-hover:bg-brand-secondary group-hover:text-white transition-colors">
