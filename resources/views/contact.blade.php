...
@section('title', __('Contactar Private Office | Intellectus'))
...
<div class="min-h-screen flex flex-col lg:flex-row">
    ...
    <div class="lg:w-5/12 bg-brand-secondary text-white relative flex flex-col justify-between p-12 lg:p-20 overflow-hidden">
        ...
        <div class="relative z-10">
            ...
            <h1 class="font-serif text-5xl lg:text-6xl leading-tight mb-8">
                {{ __('Conversas que definem') }} <span class="text-brand-primary italic">{{ __('legados.') }}</span>
            </h1>
            <p class="text-white/60 font-light leading-relaxed max-w-sm">
                {{ __('A nossa equipa, liderada por') }} <strong>Rui Moita</strong>, {{ __('assegura um acompanhamento discreto e estratégico para operações imobiliárias de alto volume.') }}
            </p>
        </div>
        ...
        <div class="relative z-10 mt-16 space-y-12">
            ...
            <div class="flex gap-6 items-start group">
                ...
                <div>
                    <h4 class="text-xs font-bold uppercase tracking-widest text-white/40 mb-2">{{ __('Sede') }}</h4>
                    <p class="font-serif text-xl">Rua José Félix Ferreira 2a</p>
                    <p class="text-sm text-white/60">2830-222 Barreiro, Portugal</p>
                </div>
            </div>
            ...
            <div class="flex gap-6 items-start group">
                ...
                <div>
                    <h4 class="text-xs font-bold uppercase tracking-widest text-white/40 mb-2">{{ __('Atendimento') }}</h4>
                    <a href="tel:+351963947269" class="block font-serif text-xl hover:text-brand-primary transition-colors">+351 963 947 269</a>
                    <a href="mailto:geral@imointellectus.com" class="block text-sm text-white/60 hover:text-white mt-1">geral@imointellectus.com</a>
                </div>
            </div>
    ...
    <div class="lg:w-7/12 bg-white flex items-center justify-center p-8 lg:p-20">
        <div class="w-full max-w-xl">
            ...
            <div class="mb-12">
                <h3 class="font-serif text-3xl text-brand-secondary mb-4">{{ __('Formulário de Contacto') }}</h3>
                <p class="text-gray-500 font-light text-sm">
                    {{ __('Preencha os campos abaixo. A nossa equipa entrará em contacto num prazo máximo de 24 horas úteis.') }}
                </p>
            ...
            <form action="{{ route('contact.send') }}" method="POST" class="space-y-8">
                ...
                {{-- Nome --}}
                <div class="group">
                    <label class="block text-[10px] uppercase tracking-widest text-gray-400 mb-2 group-focus-within:text-brand-secondary transition-colors">{{ __('Nome Completo') }}</label>
                    <input type="text" name="name" required 
                           ...
                           placeholder="{{ __('Ex: Rui Moita') }}">
                </div>

                {{-- Contactos (Grid) --}}
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="group">
                        <label class="block text-[10px] uppercase tracking-widest text-gray-400 mb-2 group-focus-within:text-brand-secondary transition-colors">{{ __('Email') }}</label>
                    ...
                    <div class="group">
                        <label class="block text-[10px] uppercase tracking-widest text-gray-400 mb-2 group-focus-within:text-brand-secondary transition-colors">{{ __('Telemóvel') }}</label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="group">
                        <label class="block text-[10px] uppercase tracking-widest text-gray-400 mb-2">{{ __('Assunto') }}</label>
                        <select name="subject" class="w-full border-0 border-b border-gray-200 py-3 text-brand-secondary focus:ring-0 focus:border-brand-primary bg-transparent cursor-pointer">
                            <option value="Comprar">{{ __('Comprar Imóvel') }}</option>
                            <option value="Vender">{{ __('Vender Imóvel') }}</option>
                            <option value="Investimento">{{ __('Investimento') }}</option>
                    ...
                    <div class="group">
                        ...
                        <select name="timeline" class="w-full border-0 border-b border-gray-200 py-3 text-brand-secondary focus:ring-0 focus:border-brand-primary bg-transparent cursor-pointer">
                            ...
                            <option value="3 Meses">{{ __('Até 3 Meses') }}</option>
                            <option value="6 Meses">{{ __('Até 6 Meses') }}</option>
                            <option value="Indefinido">{{ __('Indefinido') }}</option>
                        </select>
                    </div>
                </div>
                ...
                {{-- Mensagem --}}
                <div class="group">
                    <label class="block text-[10px] uppercase tracking-widest text-gray-400 mb-2 group-focus-within:text-brand-secondary transition-colors">{{ __('Mensagem Privada') }}</label>
                    <textarea name="message" rows="3" required 
                              ...
                              placeholder="{{ __('Descreva brevemente o seu objetivo...') }}"></textarea>
                </div>
                ...
                <div class="pt-4">
                    <label class="flex items-start gap-3 cursor-pointer group">
                        ...
                        <span class="text-xs text-gray-400 leading-relaxed group-hover:text-gray-500 transition-colors">
                            {{ __('Li e aceito a') }} <a href="{{ route('terms') }}" class="underline hover:text-brand-secondary">{{ __('Política de Privacidade') }}</a>. {{ __('Compreendo que os dados fornecidos serão tratados exclusivamente pela Intellectus para fins de resposta.') }}
                        </span>
                    </label>
                </div>
