...
@section('title', __('Recrutamento'))

@section('content')
<div class="flex flex-col lg:flex-row min-h-screen">
    
    {{-- Lado Esquerdo: Imagem / Info --}}
    {{-- ALTERAÇÃO: Mudei de py-20 para pt-32 pb-20 para empurrar o conteúdo para baixo da header --}}
    <div class="lg:w-1/2 bg-brand-secondary text-white relative flex flex-col justify-center px-8 pt-32 pb-20 lg:p-24 overflow-hidden">
        ...
        <div class="relative z-10 max-w-xl">
            <p class="text-brand-accent uppercase tracking-[0.3em] text-xs font-bold mb-6">{{ __('Carreiras') }}</p>
            <h1 class="font-serif text-5xl lg:text-6xl mb-8 leading-tight">
                {{ __('Defina o padrão da excelência.') }}
            </h1>
            <p class="text-lg text-white/80 font-light leading-relaxed mb-10">
                {{ __('Na Intellectus, não vendemos apenas imóveis; gerimos património e construímos legados. Se procura um ambiente de alta performance, suporte tecnológico de ponta e uma marca de prestígio, o seu lugar é aqui.') }}
            </p>
            
            <ul class="space-y-4 font-light text-white/70">
                <li class="flex items-center gap-4">
                    <span class="w-2 h-2 bg-brand-accent rounded-full"></span>
                    {{ __('Comissionamento acima da média de mercado.') }}
                </li>
                <li class="flex items-center gap-4">
                    <span class="w-2 h-2 bg-brand-accent rounded-full"></span>
                    {{ __('Formação contínua em Luxury Real Estate.') }}
                </li>
                <li class="flex items-center gap-4">
                    <span class="w-2 h-2 bg-brand-accent rounded-full"></span>
                    {{ __('Acesso a carteira de clientes exclusiva.') }}
                </li>
            </ul>
        </div>
    </div>
    ...
    {{-- ALTERAÇÃO: Igualmente aqui, pt-32 para compensar o header fixo no mobile --}}
    <div class="lg:w-1/2 bg-white flex flex-col justify-center px-8 pt-32 pb-20 lg:p-24">
        <div class="max-w-lg mx-auto w-full">
            <h2 class="font-serif text-3xl text-brand-secondary mb-2">{{ __('Candidatura Espontânea') }}</h2>
            <p class="text-sm text-gray-400 mb-10">{{ __('Preencha o formulário abaixo. A confidencialidade é total.') }}</p>
            ...
            <form action="{{ route('recruitment.submit') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" class="w-full border-b border-gray-200 focus:border-brand-secondary focus:ring-0 px-0 py-3 text-brand-secondary font-medium transition-colors placeholder-transparent" placeholder="{{ __('Nome') }}" required>
                </div>
                ...
                <div>
                    <label class="block text-[10px] font-bold uppercase text-gray-400 mb-2 tracking-wider">{{ __('Perfil LinkedIn (Opcional)') }}</label>
                ...
                <div>
                    <label class="block text-[10px] font-bold uppercase text-gray-400 mb-4 tracking-wider">{{ __('Anexar CV (PDF)') }}</label>
                    <label class="flex items-center gap-4 cursor-pointer group">
                        ...
                        <span class="text-sm text-gray-500 group-hover:text-brand-secondary transition-colors">{{ __('Clique para carregar ficheiro') }}</span>
