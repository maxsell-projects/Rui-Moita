...
@section('title', $property->title . ' | Marco Moura Private Office')
                ...
                <div class="max-w-3xl" data-aos="fade-up">
                    <div class="flex flex-wrap items-center gap-3 mb-6">
                        <span class="bg-brand-primary text-white px-4 py-1 text-[10px] uppercase tracking-[0.2em] font-bold shadow-lg">
                            {{ $property->type }} • {{ $property->status === 'available' ? __('Disponível') : __('Reservado') }}
                        </span>
                        ...
                        @if($property->reference_code)
                            <span class="bg-white/10 backdrop-blur-md text-white px-4 py-1 text-[10px] uppercase tracking-widest font-bold border-l-2 border-brand-sand">
                                {{ __('Ref.') }} {{ $property->reference_code }}
                            </span>
                        @endif
                    </div>
        ...
        <div class="lg:col-span-8 space-y-16">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                <div class="text-center p-6 bg-white shadow-sm border-t-2 border-brand-sand group hover:-translate-y-1 transition-transform duration-300">
                    ...
                    <span class="text-[10px] uppercase tracking-[0.2em] text-gray-400">{{ __('Quartos') }}</span>
                </div>
                <div class="text-center p-6 bg-white shadow-sm border-t-2 border-brand-sand group hover:-translate-y-1 transition-transform duration-300">
                    ...
                    <span class="text-[10px] uppercase tracking-[0.2em] text-gray-400">{{ __('Casas de Banho') }}</span>
                </div>
                <div class="text-center p-6 bg-white shadow-sm border-t-2 border-brand-sand group hover:-translate-y-1 transition-transform duration-300">
                    ...
                    <span class="text-[10px] uppercase tracking-[0.2em] text-gray-400">{{ __('m² Área Útil') }}</span>
                </div>
                <div class="text-center p-6 bg-white shadow-sm border-t-2 border-brand-sand group hover:-translate-y-1 transition-transform duration-300">
                    ...
                    <span class="text-[10px] uppercase tracking-[0.2em] text-gray-400">{{ __('Lugares') }}</span>
                </div>
            </div>
            ...
            <div data-aos="fade-up">
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-y-4 gap-x-8">
                    @php
                        $features = [
                            ['label' => __('Piscina Privada'), 'active' => $property->has_pool],
                            ['label' => __('Jardim Exclusivo'), 'active' => $property->has_garden],
                            ['label' => __('Elevador'), 'active' => $property->has_lift],
                            ['label' => __('Terraço / Varanda'), 'active' => $property->has_terrace],
                            ['label' => __('Ar Condicionado'), 'active' => $property->has_air_conditioning],
                            ['label' => __('Mobilado'), 'active' => $property->is_furnished],
                            ['label' => __('Cozinha Equipada'), 'active' => $property->is_kitchen_equipped],
                        ];
                    @endphp
                    ...
                    @if($property->floor) <div class="flex items-center gap-3 text-gray-700 font-light"><span class="text-brand-sand">▪</span> {{ __('Andar') }}: {{ $property->floor }}</div> @endif
                    @if($property->orientation) <div class="flex items-center gap-3 text-gray-700 font-light"><span class="text-brand-sand">▪</span> {{ __('Orientação') }}: {{ $property->orientation }}</div> @endif
                    @if($property->energy_rating) <div class="flex items-center gap-3 text-gray-700 font-light"><span class="text-brand-sand">▪</span> {{ __('Cert. Energética') }}: <span class="font-bold border px-1 border-gray-300">{{ $property->energy_rating }}</span></div> @endif
                </div>
            </div>
                ...
                <div class="pt-8" data-aos="fade-up">
                    ...
                    @else
                        <a href="{{ $property->video_url }}" target="_blank" class="inline-flex items-center gap-4 bg-brand-secondary text-white px-8 py-5 text-xs font-bold uppercase tracking-widest hover:bg-brand-primary transition-colors group">
                            <span>{{ __('Assistir Vídeo do Imóvel') }}</span>
                ...
                <div class="bg-white p-10 shadow-2xl border-t-4 border-brand-primary relative overflow-hidden" data-aos="fade-left">
                    ...
                    <div class="relative z-10">
                        <div class="flex justify-between items-end mb-4 border-b border-gray-100 pb-4">
                            <p class="text-[10px] text-gray-400 uppercase tracking-[0.2em]">{{ __('Investimento') }}</p>
                            @if($property->reference_code)
                                <p class="text-[10px] font-bold text-brand-primary uppercase tracking-widest">{{ __('Ref') }}: {{ $property->reference_code }}</p>
                            @endif
                        </div>
                        ...
                        <div class="mt-8 pt-8 border-t border-gray-100 text-center">
                            <p class="text-[10px] text-gray-400 uppercase tracking-widest mb-4">{{ __('Partilhar Oportunidade') }}</p>
